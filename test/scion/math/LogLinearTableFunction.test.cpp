...
SCENARIO( "LogLinearTableFunction" ) {
  ...
  GIVEN( "tabulated x,f(y) data" ) {
    ...
    WHEN( "the data is given explicitly using vectors" ) {
      ...
      LogLinearTableFunction< double > chunk( std::move( x ), std::move( f ) );
      ...
      THEN( "a LogLinearTable can be evaluated" ) {
        ...
        CHECK_THAT( 0.49749371855331, WithinRel( chunk( 1.5, -0.5 ) ) );
        CHECK_THAT( 0.47196398167657, WithinRel( chunk( 2.5, -0.5 ) ) );
        CHECK_THAT( 0.36742346141748, WithinRel( chunk( 3.5, -0.5 ) ) );
      } // THEN
    } // WHEN
  ...
  GIVEN( "invalid data for a LogLinearTable object" ) {
    ...
    WHEN( "there are not enough values in the x or f(y) grid" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( LogLinearTableFunction< double >( xempty, fempty ) );
        CHECK_THROWS( LogLinearTableFunction< double >( xone, fone ) );
        CHECK_THROWS( LogLinearTableFunction< double >( xempty, fone ) );
        CHECK_THROWS( LogLinearTableFunction< double >( xone, fempty ) );
      } // THEN
    } // WHEN
    ...
    WHEN( "the x and f(y) grid do not have the same number of points" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( LogLinearTableFunction< double >( std::move( x ), std::move( f ) ) );
      } // THEN
    } // WHEN

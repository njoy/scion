  ...
  GIVEN( "Legendre coefficients for an expansion" ) {
    ...
    WHEN( "the data is given explicitly" ) {
      ...
      THEN( "a LegendreSeries can be differentiated" ) {
        ...
        CHECK_THAT(  15., WithinRel( first.coefficients()[0] ) );
        CHECK_THAT( -14., WithinRel( first.coefficients()[1] ) );
        CHECK_THAT(   2., WithinRel( first.coefficients()[2] ) );
      ...
      THEN( "a LegendreSeries can be integrated" ) {
        ...
        CHECK_THAT( 2.3833333333333333, WithinRel( primitive.coefficients()[0] ) );
        CHECK_THAT( -9.4              , WithinRel( primitive.coefficients()[1] ) );
        CHECK_THAT(  4.809523809523809, WithinRel( primitive.coefficients()[2] ) );
        CHECK_THAT( -0.933333333333333, WithinRel( primitive.coefficients()[3] ) );
        ...
        CHECK( 4 == primitive.order() );
        CHECK( 5 == primitive.coefficients().size() );
        CHECK_THAT( -15.2             , WithinRel( primitive.coefficients()[0] ) );
        CHECK_THAT(  -9.4             , WithinRel( primitive.coefficients()[1] ) );
        CHECK_THAT(  4.809523809523809, WithinRel( primitive.coefficients()[2] ) );
        CHECK_THAT( -0.933333333333333, WithinRel( primitive.coefficients()[3] ) );
        CHECK_THAT(  0.057142857142857, WithinRel( primitive.coefficients()[4] ) );
      ...
      THEN( "a LegendreSeries can be linearised" ) {
        ...
        CHECK_THAT( -0.047855377197266, WithinRel( linear.y()[18], 1e-12 ) );
        CHECK_THAT( -0.023682117462158, WithinRel( linear.y()[19], 1e-12 ) );
        CHECK_THAT(  0.0              , WithinAbs( linear.y()[20], 1e-12 ) );
    ...
    WHEN( "using constant assignment" ) {
      ...
      THEN( "an LegendreSeries can be constructed and  members can be tested" ) {

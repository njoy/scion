...
SCENARIO( "LinearLinearTableFunction" ) {
  ...
  GIVEN( "tabulated x,f(y) data" ) {
    ...
    WHEN( "the data is given explicitly using vectors" ) {
      ...
      LinearLinearTableFunction< double > chunk( std::move( x ), std::move( f ) );
  ...
  GIVEN( "invalid data for a LinearLinearTable object" ) {
    ...
    WHEN( "there are not enough values in the x or f(y) grid" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( LinearLinearTableFunction< double >( xempty, fempty ) );
        CHECK_THROWS( LinearLinearTableFunction< double >( xone, fone ) );
        CHECK_THROWS( LinearLinearTableFunction< double >( xempty, fone ) );
        CHECK_THROWS( LinearLinearTableFunction< double >( xone, fempty ) );
      } // THEN
    } // WHEN
    ...
    WHEN( "the x and f(y) grid do not have the same number of points" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( LinearLinearTableFunction< double >( std::move( x ), std::move( f ) ) );
      } // THEN
    } // WHEN
